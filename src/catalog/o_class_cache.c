...
static Oid	o_class_cache_get_link_oid(Pointer entry);
static void o_class_cache_fill_entry(Pointer *entry_ptr, Oid datoid,
									  Oid classoid, XLogRecPtr insert_lsn,
									  Pointer arg);
...
O_SYS_CACHE_FUNCS(class_cache, OClass);
...
static OSysCacheFuncs class_cache_funcs =
{
	...
	.fastcache_get_link_oid = o_class_cache_get_link_oid
};
...
void
o_class_cache_fill_entry(Pointer *entry_ptr, Oid datoid, Oid classoid,
			  			  XLogRecPtr insert_lsn, Pointer arg)
{
	...
	for (i = 0; i < o_class->natts; i++)
	{
		...
		process = !class_attr->attisdropped &&
				  (!sys_cache || processed < CATCACHE_MAXKEYS);
		if (process)
		{
			processed++;
			o_composite_type_element_save(datoid, class_attr->atttypid,
										  insert_lsn);
		}
	}
 ...
 */
Oid
o_class_cache_get_link_oid(Pointer entry)
{
	OClass    *o_class = (OClass *) entry;
...
TupleDesc
o_class_cache_search_tupdesc(Oid classoid)
{
	...
	o_class = o_class_cache_search(datoid, classoid, cur_lsn);
	if (o_class)
	{
