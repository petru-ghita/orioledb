...
static void o_proc_cache_fill_entry(Pointer *entry_ptr, Oid datoid, Oid typoid,
									XLogRecPtr insert_lsn, Pointer arg);
O_SYS_CACHE_FUNCS(proc_cache, OProc);
...
void
o_proc_cache_fill_entry(Pointer *entry_ptr, Oid datoid, Oid procoid,
			  			XLogRecPtr insert_lsn, Pointer arg)
{
	HeapTuple				proctup;
	...
	else
	{
		...
		o_proc->cxt = AllocSetContextCreate(proc_cache->mcxt,
											"o_proc mcxt",
	...
	InitFunctionCallInfoData(*fcinfo, finfo, 2, fncollation, NULL, NULL);
...
o_proc_cache_serialize_entry(Pointer entry, int *len)
{
	...
	if (o_proc->prolang == SQLlanguageId)
	{
		...
		appendBinaryStringInfo(&str, (Pointer) sql_func,
							   offsetof(sql_func_data, fname));
		o_serialize_string(sql_func->fname, &str);
		o_serialize_string(sql_func->src, &str);
		appendBinaryStringInfo(&str, ((Pointer) sql_func) +
									 offsetof(sql_func_data, rettype),
		...
		appendBinaryStringInfo(&str, (Pointer) sql_func->jf_atts,
							   sizeof(jf_atttype) * sql_func->jf_natts);
		appendBinaryStringInfo(&str, (Pointer) sql_func->argtypes,
							   sizeof(Oid) * sql_func->nargs);

		for (i = 0; i < sql_func->nargs; i++)
		{
...
o_proc_cache_deserialize_entry(MemoryContext mcxt, Pointer data, Size length)
{
	...
	o_proc = (OProc *) palloc(sizeof(OProc));
	len = offsetof(OProc, prosrc);
	...
	if (o_proc->prolang == SQLlanguageId)
	{
		...
		len = offsetof(sql_func_data, fname);
		Assert((ptr - data) + len <= length);
		memcpy(sql_func, ptr, len);
		ptr += len;

		sql_func->fname = o_deserialize_string(&ptr);
		sql_func->src = o_deserialize_string(&ptr);

		len = offsetof(sql_func_data, jf_targetList) -
			  offsetof(sql_func_data, rettype);
		Assert((ptr - data) + len <= length);
		memcpy(((Pointer) sql_func) + offsetof(sql_func_data, rettype),
			   ptr, len);
		ptr += len;

		sql_func->jf_targetList = (List *) o_deserialize_node(&ptr);

		len = sizeof(jf_atttype) * sql_func->jf_natts;
		Assert((ptr - data) + len <= length);
		sql_func->jf_atts = palloc0(len);
		memcpy(sql_func->jf_atts, ptr, len);
		ptr += len;

		len = sizeof(Oid) * sql_func->nargs;
		Assert((ptr - data) + len <= length);
		sql_func->argtypes = palloc0(len);
		memcpy(sql_func->argtypes, ptr, len);
		ptr += len;

		len = sizeof(char *) * sql_func->nargs;
...
o_prepare_sql_fn_parse_info(sql_func_data *sql_func,
							Node *call_expr,
							Oid inputCollation)
{
	...
	pinfo->fname = pstrdup(sql_func->fname);
	 ...
	 * polymorphic types.
	 */
	pinfo->nargs = nargs = sql_func->nargs;
	if (nargs > 0)
	{
		Oid		   *argOidVect;
		int			argnum;
		...
		memcpy(argOidVect, sql_func->argtypes, nargs * sizeof(Oid));
				...
				sql_func_data *out_sql_func, MemoryContext out_sql_func_cxt)
{
	...
	if (o_proc)
	{
		fcache->fname = pstrdup(sql_func->fname);
	}
	else
	{
		/*
	...
	if (sql_func)
	{
		fcache->typlen = sql_func->typlen;
		fcache->typbyval = sql_func->typbyval;
	...
	if (o_proc)
	{
		...
		*/
		fcache->pinfo = o_prepare_sql_fn_parse_info(sql_func,
													finfo->fn_expr,
													collation);
	}
	else
	...
	if (out_sql_func)
	{
		...
		out_sql_func->fname = pstrdup(fcache->fname);
		out_sql_func->src = pstrdup(fcache->src);
		out_sql_func->rettype = fcache->rettype;
		out_sql_func->typlen = fcache->typlen;
		out_sql_func->typbyval = fcache->typbyval;
		...
		out_sql_func->jf_targetList = copyObject(fcache->junkFilter->jf_targetList);

		out_sql_func->nargs = procedureStruct->pronargs;
		out_sql_func->argtypes = palloc0(out_sql_func->nargs * sizeof(Oid));
		memcpy(out_sql_func->argtypes, fcache->pinfo->argtypes,
			   out_sql_func->nargs * sizeof(Oid));
		for (i = 0; i < out_sql_func->nargs; i++)
		{
			o_type_cache_add_if_needed(datoid, out_sql_func->argtypes[i],
									   cur_lsn, NULL);
		}
		o_class_cache_add_if_needed(datoid, TypeRelationId, cur_lsn,
									(Pointer) &arg);
		out_sql_func->argnames = palloc0(out_sql_func->nargs * sizeof(char *));
		for (i = 0; i < out_sql_func->nargs; i++)
		{
			out_sql_func->argnames[i] = pstrdup(fcache->pinfo->argnames[i]);
		}

		out_sql_func->jf_natts = fcache->junkFilter->jf_cleanTupType->natts;
		out_sql_func->jf_atts = palloc0(sizeof(jf_atttype) *
		...
		foreach(l, out_sql_func->jf_targetList)
		{
			...
			jf_att = &out_sql_func->jf_atts[cur_resno - 1];
			att = &fcache->junkFilter->jf_cleanTupType->attrs[cur_resno - 1];
			jf_att->typlen = att->attlen;
			jf_att->typbyval = att->attbyval;
			jf_att->typalign = att->attalign;
			jf_att->typstorage = att->attstorage;
			jf_att->typcollation = att->attcollation;
			cur_resno++;
		}
...
o_fmgr_sql(PG_FUNCTION_ARGS)
{
	...
	while (es)
	{
		...
		{
			ResourceOwner	my_owner = NULL;
			// Oid				my_userid = InvalidOid,
			// 				cur_userid;
			...
			// GetUserIdAndContext(&cur_userid, &sec_def_context);

			// if (!OidIsValid(cur_userid))
			// {
			// 	my_userid = ;
			// }
